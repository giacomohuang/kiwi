...
function main(args) {
  ...
  let isText = Array.prototype.indexOf.call(buffer, 0) === -1;
  let content = isText ? buffer.toString() : new Uint8Array(buffer);
  try {
    var parsed = typeof content === "string" ? parseSchema(content) : decodeBinarySchema(new ByteBuffer(content));
  ...
  if (flags["--to-json"] !== null) {
    ...
    writeFileString(flags["--to-json"] + ".json", JSON.stringify(compiled["decode" + rootType](
  ...
  if (flags["--from-json"] !== null) {
    ...
    writeFileBuffer(flags["--from-json"] + ".bin", Buffer.from(compiled["encode" + rootType](
